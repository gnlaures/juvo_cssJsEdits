<?php
$city = $map_atts['areaDestination'];
$province = $map_atts['province'];
$region = $map_atts['region'];
$label = $map_atts['label'];
$labelexact = $map_atts['labelexact'];
$nouse = '';
$querystr = '';
...
if (!empty($region)) {
	$querystr .= '&region=' . $region;
}
if (!empty($label)) {
	$querystr .= '&label=' . $label;
}
if (!empty($labelexact)) {
	$querystr .= '&labelexact=' . $labelexact;
}
$uriSegments = explode("/", $_SERVER['REQUEST_URI']);
if (!empty($uriSegments[1])) {
	$querystr .= '&page=' . $uriSegments[1];
}
?>
