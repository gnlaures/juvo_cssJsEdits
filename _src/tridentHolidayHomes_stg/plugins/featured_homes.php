...
$parameters = ['display_type' => 'featured', 'page_num' => 1, 'page_size' => 10];
if (!empty($homes_atts['county'])) {
	$parameters['city'] = $homes_atts['county'];
}
if (!empty($homes_atts['region'])) {
	$parameters['region'] = $homes_atts['region'];
}
if (!empty($homes_atts['propids'])) {
	$parameters['propids'] = $homes_atts['propids'];
}
$accommodations = getAccommodations($parameters);
if (!empty($accommodations)) {
?>
<div class="featured-container">
	<div id="sHomes" class="swiper featuredRecentlyViewedSwiper sHomes">
		<ul class="swiper-wrapper prop-grid-view featured-homes">
		<?php 
		$index = 0;
		$pictureLimit = 4;
		foreach($accommodations as $accommodation) {
			...
			$home = '<li class="swiper-slide-featured-recentlyviewed featured-recentlyviewed-wrapper" data-propid="' . $accommodationId . '">
			...
			if (!empty($accommodation->OccupantsCapacity)) {
				...
				if (!empty($accommodation->MinimumOccupation)) {
					$home .= '<span class="occupants"><span><span class="svg-occupants" aria-hidden="true"></span><span class="tot-text" aria-hidden="true">' . $accommodation->MinimumOccupation . '</span><span class="tot-text" aria-hidden="true">' . $accommodation->MinimumOccupation . '</span><span class="visually-hidden">' . $accommodation->MinimumOccupation . ' Occupant' . ($accommodation->MinimumOccupation != 1 ? 's' : '') . '</span></span></span>';
				}
			}
...
<?php
	...
	echo '<p class="textwhite">There are currently no featured homes set here...</p>';
}
?>